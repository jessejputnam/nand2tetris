...
class SymbolTable:
    ...
    def define(self, name: str, type: str, kind: kind_input) -> None:
        ...
        idx = self.var_count(kind.lower())
        self.sub_vars[name] = {"type": type, "kind": kind.lower(), "idx": idx}
        return
    ...
    def var_count(self, kind: str) -> int:
        ...
        if kind not in ["STATIC", "FIELD", "ARG", "VAR"]:
            raise Exception(
                f"Unrecognized variable type <{kind}> updating Symbol Table."
            )

        scope = self.class_vars if kind in ["STATIC", "FIELD"] else self.sub_vars
        return len([x for x in scope.values() if x["kind"] == kind])
    ...
    def kind_of(self, name: str) -> kind_output:
        ...
        if name in self.sub_vars:
            return self.sub_vars[name]["kind"]
        elif name in self.class_vars:
            return self.class_vars[name]["kind"]
        else:
            return "NONE"
